...
public class PlanarReflection : MonoBehaviour
{
    ...
    RenderTexture rtTempA, rtTempB;
    void PostProcessTexture(Camera cam, RenderTexture source, RenderTexture dest)
    {
        //降低分辨率 优化性能
        int rtW = source.width / (int)_downsample;
        int rtH = source.height / (int)_downsample;

        if (ModeGraphics)
        {
            if (Time.time % 2 == 0) return;
            BlurMaterial.SetFloat("_Offset", _blurSize);
            rtTempA = RenderTexture.GetTemporary(rtW, rtH, 0, source.format);
            ...
            for (int i = 0; i < _blurIterations; i++)
            {
                if (0 == i)
                {
                    Graphics.Blit(source, rtTempA, BlurMaterial, 0);
                    Graphics.Blit(rtTempA, rtTempB, BlurMaterial, 0);
                }
                else
                {

                    Graphics.Blit(rtTempB, rtTempA, BlurMaterial, 0);
                    Graphics.Blit(rtTempA, rtTempB, BlurMaterial, 0);
                }
            }
        ...
        else
        {
            ...
            buf.Blit(source, blurredID);

            for (int i = 0; i < _blurIterations; i++)
            {
                float iterationOffs = (i * 1.0f);
                BlurMaterial.SetFloat("_Offset", iterationOffs / _downsample + _blurSize);
